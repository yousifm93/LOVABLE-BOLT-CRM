-- Create audit logging system for dashboard enhancements (fixed enum values)
do $$ begin
  ...
  with base as (
    select 
      case 
        ...
        when status = 'Pending App' then 'pending_app'
        when status = 'Nurture' then 'screening'  
        when status = 'Needs Attention' then 'pre_qualified'
        else 'active'
      end as stage,
      case 
        when status in ('Working on it', 'Pending App', 'Nurture', 'Needs Attention') then 'nurture'
