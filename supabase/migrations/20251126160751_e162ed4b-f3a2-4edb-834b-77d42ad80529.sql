-- Add missing audit triggers for Activity Monitor

-- 1. Add audit trigger for buyer_agents (category: contacts)
DROP TRIGGER IF EXISTS audit_buyer_agents_changes ON public.buyer_agents;
CREATE TRIGGER audit_buyer_agents_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.buyer_agents
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log('contacts', 'buyer_agents');

-- 2. Add audit trigger for lenders (category: contacts)
...
CREATE TRIGGER audit_lenders_changes
  ...
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log('contacts', 'lenders');

-- 3. Add audit trigger for lead_conditions (category: pipeline)
DROP TRIGGER IF EXISTS audit_lead_conditions_changes ON public.lead_conditions;
CREATE TRIGGER audit_lead_conditions_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.lead_conditions
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log('pipeline', 'lead_conditions');

-- 4. Add audit trigger for call_logs (category: pipeline)
DROP TRIGGER IF EXISTS audit_call_logs_changes ON public.call_logs;
CREATE TRIGGER audit_call_logs_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.call_logs
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log('pipeline', 'call_logs');

-- 5. Add audit trigger for email_logs (category: pipeline)
DROP TRIGGER IF EXISTS audit_email_logs_changes ON public.email_logs;
CREATE TRIGGER audit_email_logs_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.email_logs
  FOR EACH ROW EXECUTE FUNCTION fn_audit_log('pipeline', 'email_logs');

-- 6. Create enhanced dashboard_activity_details function
CREATE OR REPLACE FUNCTION dashboard_activity_details(
  _from timestamptz, 
  _to timestamptz, 
  _category text,
