-- Insert sample active borrowers with proper account_id and created_by
-- First, get a sample account_id and user_id from existing data
DO $$
DECLARE
    sample_account_id uuid;
    sample_user_id uuid;
BEGIN
    -- Get the first available account_id and user_id
    SELECT account_id INTO sample_account_id FROM public.profiles LIMIT 1;
    SELECT user_id INTO sample_user_id FROM public.profiles LIMIT 1;
    
    -- If no profiles exist, create a default account_id
    IF sample_account_id IS NULL THEN
        sample_account_id := gen_random_uuid();
    END IF;
    
    -- If no user exists, create a default user_id  
    ...
    INSERT INTO public.leads (
      ...
      lead_on_date, status, lead_strength, notes, account_id, created_by
    ) VALUES 
    (
      ...
      'First-time homebuyer, pre-approved for $700k', sample_account_id, sample_user_id
    ),
    (
      ...
      'Cash-out refinance, excellent credit score', sample_account_id, sample_user_id
    ),
    (
      ...
      'Moving from out of state, needs quick closing', sample_account_id, sample_user_id
    ),
    (
      ...
      'Investment property purchase, experienced investor', sample_account_id, sample_user_id
    ),
    (
      ...
      'Luxury home purchase, bridge loan needed', sample_account_id, sample_user_id
    );
END $$;