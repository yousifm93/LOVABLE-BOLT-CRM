...
-- The issue: when inserting into tasks table, the 'status' column was null
-- because the function wasn't setting it correctly (likely using a variable with wrong type)
...
BEGIN
  ...
  LOOP
    ...
    -- Insert the new task with explicit status value
    INSERT INTO tasks (
      borrower_id,
      title,
      description,
      ...
      'To Do',  -- Explicit status value, not a variable
      priority_value,
      due_date_value,
      assignee_id_value,
      automation.id
