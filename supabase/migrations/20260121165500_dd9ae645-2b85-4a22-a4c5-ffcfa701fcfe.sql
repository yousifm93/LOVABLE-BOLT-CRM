-- Fix task automation trigger - remove reference to non-existent columns
...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- Only execute if pipeline_stage_id has changed
  IF OLD.pipeline_stage_id IS DISTINCT FROM NEW.pipeline_stage_id THEN
    ...
    -- Find all active automations for the new pipeline stage
    FOR automation IN
      SELECT *
    ...
    LOOP
      BEGIN
        ...
        INSERT INTO public.tasks (
          ...
          due_date, status, created_by, automation_id, completion_requirement_type
        ) VALUES (
        ...
        ) VALUES (automation.id, NEW.id, new_task_id, NOW(), true);

      EXCEPTION WHEN OTHERS THEN
        -- Log failed execution
        INSERT INTO public.task_automation_executions (
          automation_id, lead_id, task_id, executed_at, success, error_message
        ) VALUES (automation.id, NEW.id, NULL, NOW(), false, SQLERRM);
      END;
    END LOOP;
  END IF;
  RETURN NEW;
...
CREATE TRIGGER trigger_pipeline_stage_automations
