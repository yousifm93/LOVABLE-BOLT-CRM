...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log("Sending loan estimate email to:", primaryEmail, secondaryEmail ? `and ${secondaryEmail}` : "");

    // Prepare recipient list - avoid duplicates, handle comma-separated secondaryEmail
    const recipients = [primaryEmail];
    if (secondaryEmail && secondaryEmail.trim()) {
      const secondaryEmails = secondaryEmail.split(',').map(e => e.trim()).filter(e => e && e !== primaryEmail);
      recipients.push(...secondaryEmails);
    }
    ...
    const sendGridResponse = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        ...
        subject: `Loan Estimate - ${customerName}`,
        content: [{
          type: "text/html",
          value: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #333;">Your Loan Estimate</h2>
              <p style="color: #666; line-height: 1.6;">Please find your loan estimate attached.</p>
              <p style="color: #666; line-height: 1.6;">This estimate provides a breakdown of your expected costs and monthly payments. Please review carefully and let us know if you have any questions.</p>
    ...
    console.error("Error in send-loanestimate-email function:", error);
    return new Response(
